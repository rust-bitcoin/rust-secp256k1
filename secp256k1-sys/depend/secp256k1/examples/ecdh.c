...
int main(void) {
    ...
    int return_val;
    size_t len;
    rustsecp256k1_v0_10_0_pubkey pubkey1;
    ...
    return_val = rustsecp256k1_v0_10_0_ec_pubkey_create(ctx, &pubkey1, seckey1);
    assert(return_val);
    return_val = rustsecp256k1_v0_10_0_ec_pubkey_create(ctx, &pubkey2, seckey2);
    assert(return_val);
    ...
    return_val = rustsecp256k1_v0_10_0_ec_pubkey_serialize(ctx, compressed_pubkey2, &len, &pubkey2, SECP256K1_EC_COMPRESSED);
    assert(return_val);
    /* Should be the same size as the size of the output, because we passed a 33 byte array. */
    ...
    return_val = rustsecp256k1_v0_10_0_ecdh(ctx, shared_secret1, &pubkey2, seckey1, NULL, NULL);
    assert(return_val);

    /* Perform ECDH with seckey2 and pubkey1. Should never fail with a verified
     * seckey and valid pubkey */
    return_val = rustsecp256k1_v0_10_0_ecdh(ctx, shared_secret2, &pubkey1, seckey2, NULL, NULL);
    assert(return_val);
