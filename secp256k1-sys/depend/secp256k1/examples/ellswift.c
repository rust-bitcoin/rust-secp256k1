 ...
 *  in include/rustsecp256k1_v0_10_0_ellswift.h and doc/ellswift.md.
 */
...
int main(void) {
    ...
    while (1) {
        ...
        if (rustsecp256k1_v0_10_0_ec_seckey_verify(ctx, seckey1) && rustsecp256k1_v0_10_0_ec_seckey_verify(ctx, seckey2)) {
            break;
        }
    }
    ...
    return_val = rustsecp256k1_v0_10_0_ellswift_create(ctx, ellswift_pubkey1, seckey1, auxrand1);
    assert(return_val);
    return_val = rustsecp256k1_v0_10_0_ellswift_create(ctx, ellswift_pubkey2, seckey2, auxrand2);
    assert(return_val);
    ...
    return_val = rustsecp256k1_v0_10_0_ellswift_xdh(ctx, shared_secret1, ellswift_pubkey1, ellswift_pubkey2,
        seckey1, 0, rustsecp256k1_v0_10_0_ellswift_xdh_hash_function_bip324, NULL);
    assert(return_val);

    /* Perform x-only ECDH with seckey2 and ellswift_pubkey1. Should never fail
     * with a verified seckey and valid pubkey. */
    return_val = rustsecp256k1_v0_10_0_ellswift_xdh(ctx, shared_secret2, ellswift_pubkey1, ellswift_pubkey2,
        seckey2, 1, rustsecp256k1_v0_10_0_ellswift_xdh_hash_function_bip324, NULL);
    assert(return_val);
