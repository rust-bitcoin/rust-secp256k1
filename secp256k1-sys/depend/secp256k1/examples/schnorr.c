...
int main(void) {
    ...
    return_val = rustsecp256k1_v0_13_context_randomize(ctx, randomize);
    assert(return_val);
    ...
    if (!rustsecp256k1_v0_13_keypair_create(ctx, &keypair, seckey)) {
     ...
     * `rustsecp256k1_v0_13_keypair_xonly_pub` supports returning the parity for
     * other use cases such as tests or verifying Taproot tweaks.
     * This should never fail with a valid context and public key. */
    return_val = rustsecp256k1_v0_13_keypair_xonly_pub(ctx, &pubkey, NULL, &keypair);
    assert(return_val);

    /* Serialize the public key. Should always return 1 for a valid public key. */
    return_val = rustsecp256k1_v0_13_xonly_pubkey_serialize(ctx, serialized_pubkey, &pubkey);
    assert(return_val);
     ...
     *
     * We use rustsecp256k1_v0_13_tagged_sha256 to create this hash. This function expects
    ...
    return_val = rustsecp256k1_v0_13_tagged_sha256(ctx, msg_hash, tag, sizeof(tag), msg, sizeof(msg));
    assert(return_val);
     ...
     *
     * We use the rustsecp256k1_v0_13_schnorrsig_sign32 function that provides a simple
    ...
    return_val = rustsecp256k1_v0_13_schnorrsig_sign32(ctx, signature, msg_hash, &keypair, auxiliary_rand);
    assert(return_val);
     ...
     * be parsed correctly */
    if (!rustsecp256k1_v0_13_xonly_pubkey_parse(ctx, &pubkey, serialized_pubkey)) {
    ...
    is_signature_valid = rustsecp256k1_v0_13_schnorrsig_verify(ctx, signature, msg_hash, 32, &pubkey);
    ...
    /* Bonus example: if all we need is signature verification (and no key
       ...
       rustsecp256k1_v0_13_context_create(). We can simply use the static (i.e., global)
       context rustsecp256k1_v0_13_context_static. See its description in
       include/secp256k1.h for details. */
    is_signature_valid2 = rustsecp256k1_v0_13_schnorrsig_verify(rustsecp256k1_v0_13_context_static,
