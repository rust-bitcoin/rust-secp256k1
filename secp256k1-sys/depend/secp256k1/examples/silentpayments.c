...
const unsigned char* label_lookup(
    ...
    for (i = 0; i < cache->entries_used; i++) {
        if (rustsecp256k1_v0_10_0_memcmp_var(cache->entries[i].label, label33, 33) == 0) {
...
int main(void) {
    ...
    /*** Sending ***/
    {
        rustsecp256k1_v0_10_0_keypair sender_seckeys[N_INPUTS];
        const rustsecp256k1_v0_10_0_keypair *sender_seckey_ptrs[N_INPUTS];
        ...
        rustsecp256k1_v0_10_0_xonly_pubkey generated_outputs[N_OUTPUTS];
        rustsecp256k1_v0_10_0_xonly_pubkey *generated_output_ptrs[N_OUTPUTS];
        ...
        for (i = 0; i < N_INPUTS; i++) {
            ...
            if (rustsecp256k1_v0_10_0_keypair_create(ctx, &sender_seckeys[i], seckey)) {
        ...
        for (i = 0; i < N_OUTPUTS; i++) { ret =
            rustsecp256k1_v0_10_0_ec_pubkey_parse(ctx,
        ...
        for (i = 0; i < N_OUTPUTS; i++) {
            ...
            rustsecp256k1_v0_10_0_xonly_pubkey_serialize(ctx,
                xonly_print,
                &generated_outputs[i]
            );
    ...
    /*** Receiving ***/
    {
        ...
        const rustsecp256k1_v0_10_0_xonly_pubkey *tx_output_ptrs[N_OUTPUTS];
...

        {
            ...
            }
            {
                 ...
                 * called
                 * `rustsecp256k1_v0_10_0_silentpayments_recipient_create_labelled_spend_pubkey`
                 * and is using the resulting labelled spend pubkey to encode a
                 * labelled silent payments address.
                 */
                ret = rustsecp256k1_v0_10_0_silentpayments_recipient_create_label_tweak(ctx,
                    &label,
                    labels_cache.entries[0].label_tweak,
                    bob_scan_key,
                    m
                );
                rustsecp256k1_v0_10_0_ec_pubkey_serialize(ctx,
             ...
             * creates a `rustsecp256k1_v0_10_0_silentpayments_public_data` object. He uses
             ...
             * with light clients.
             */
            ret = rustsecp256k1_v0_10_0_silentpayments_recipient_public_data_create(ctx,
                &public_data,
                smallest_outpoint,
            ...
            assert(ret);
            ret = rustsecp256k1_v0_10_0_silentpayments_recipient_public_data_serialize(ctx,
                light_client_data33,
                &public_data
            );
            assert(ret);
        ...
        }
        {
             ...
             */
            ret = rustsecp256k1_v0_10_0_silentpayments_recipient_public_data_parse(ctx,
                &public_data,
                light_client_data33
            );
            assert(ret);
            ret = rustsecp256k1_v0_10_0_silentpayments_recipient_create_shared_secret(ctx,
                shared_secret,
                carol_scan_key,
                &public_data
            );
            ...
            n_found_outputs = 0;
            {
                ...
                while (1) {
                    ret = rustsecp256k1_v0_10_0_silentpayments_recipient_create_output_pubkey(ctx,
                        &potential_output,
                        shared_secret,
                        &spend_pubkey,
                        k
