 ...
 *
 *      rustsecp256k1_v0_13_silentpayments_recipient_scan_outputs(..., NULL, NULL);
 */
...
int main(void) {
    ...
    rustsecp256k1_v0_13_xonly_pubkey tx_inputs[N_INPUTS];
    const rustsecp256k1_v0_13_xonly_pubkey *tx_input_ptrs[N_INPUTS];
    rustsecp256k1_v0_13_xonly_pubkey tx_outputs[N_OUTPUTS];
    rustsecp256k1_v0_13_xonly_pubkey *tx_output_ptrs[N_OUTPUTS];
    rustsecp256k1_v0_13_silentpayments_found_output found_outputs[N_OUTPUTS];
    rustsecp256k1_v0_13_silentpayments_found_output *found_output_ptrs[N_OUTPUTS];
    rustsecp256k1_v0_13_silentpayments_prevouts_summary prevouts_summary;
    rustsecp256k1_v0_13_pubkey unlabeled_spend_pubkey;
    ...
    rustsecp256k1_v0_13_context* ctx = rustsecp256k1_v0_13_context_create(SECP256K1_CONTEXT_NONE);
    if (!fill_random(randomize, sizeof(randomize))) {
        printf("Failed to generate randomness\n");
        return EXIT_FAILURE;
    }
    /* Randomizing the context is recommended to protect against side-channel
     * leakage. See `rustsecp256k1_v0_13_context_randomize` in secp256k1.h for more
     * information about it. This should never fail. */
    ret = rustsecp256k1_v0_13_context_randomize(ctx, randomize);
    assert(ret);
    ...
    /*** Create a labeled Silent Payments address (Bob) ***/
    {
        ...
        ret = rustsecp256k1_v0_13_ec_pubkey_parse(ctx,
         ...
         */
        ret = rustsecp256k1_v0_13_silentpayments_recipient_label_create(ctx,
            &label,
        ...
        ret = rustsecp256k1_v0_13_silentpayments_recipient_label_serialize(ctx, bob_labels_cache.entries[0].label, &label);
        assert(ret);
         ...
         */
        ret = rustsecp256k1_v0_13_silentpayments_recipient_create_labeled_spend_pubkey(ctx, &labeled_spend_pubkey, &unlabeled_spend_pubkey, &label);
        if (!ret) {
            printf("Something went wrong, event with negligible probability happened.\n");
            return EXIT_FAILURE;
        }
        memcpy(bob_address[0], bob_scan_and_spend_pubkeys[0], 33);
        ret = rustsecp256k1_v0_13_ec_pubkey_serialize(ctx,
            bob_address[1],
            &len,
    ...
    /*** Sending (Alice) ***/
    {
        rustsecp256k1_v0_13_keypair sender_keypairs[N_INPUTS];
        const rustsecp256k1_v0_13_keypair *sender_keypair_ptrs[N_INPUTS];
        rustsecp256k1_v0_13_silentpayments_recipient recipients[N_OUTPUTS];
        const rustsecp256k1_v0_13_silentpayments_recipient *recipient_ptrs[N_OUTPUTS];
        ...
        for (i = 0; i < N_INPUTS; i++) {
            ...
            if (rustsecp256k1_v0_13_keypair_create(ctx, &sender_keypairs[i], seckey)) {
                sender_keypair_ptrs[i] = &sender_keypairs[i];
                ret = rustsecp256k1_v0_13_keypair_xonly_pub(
                    ctx,
                    &tx_inputs[i],
                    NULL,
                    &sender_keypairs[i]
        ...
        for (i = 0; i < N_OUTPUTS; i++) {
            printf("    ");
            ret = rustsecp256k1_v0_13_xonly_pubkey_serialize(ctx,
                serialized_xonly,
                &tx_outputs[i]
            );
            assert(ret);
    ...
    {
        {
             ...
             *     2. Call `rustsecp256k1_v0_13_silentpayments_recipient_scan_outputs`
             */
            ret = rustsecp256k1_v0_13_silentpayments_recipient_prevouts_summary_create(ctx,
                &prevouts_summary,
                smallest_outpoint,
            ...
            n_found_outputs = 0;
            ret = rustsecp256k1_v0_13_silentpayments_recipient_scan_outputs(ctx,
                found_output_ptrs, &n_found_outputs,
                (const rustsecp256k1_v0_13_xonly_pubkey**)tx_output_ptrs, N_OUTPUTS,
                bob_scan_key,
                &prevouts_summary,
                &unlabeled_spend_pubkey,
            ...
            if (n_found_outputs > 0) {
                rustsecp256k1_v0_13_keypair kp;
                rustsecp256k1_v0_13_xonly_pubkey xonly_output;
                ...
                for (i = 0; i < n_found_outputs; i++) {
                     ...
                     * been added to the tweak in `rustsecp256k1_v0_13_silentpayments_found_output`.
                     *
                    ...
                    ret = rustsecp256k1_v0_13_ec_seckey_tweak_add(ctx, full_seckey, found_outputs[i].tweak);
                    ret &= rustsecp256k1_v0_13_keypair_create(ctx, &kp, full_seckey);
                    ret &= rustsecp256k1_v0_13_keypair_xonly_pub(
                        ctx,
                        &xonly_output,
                        NULL,
                     ...
                     */
                    assert(ret);
                    assert(rustsecp256k1_v0_13_xonly_pubkey_cmp(ctx, &xonly_output, &found_outputs[i].output) == 0);
        ...
        }
        {
             ...
             * the first call to `rustsecp256k1_v0_13_silentpayments_recipient_scan_outputs` above. */
            for (i = 0; i < N_OUTPUTS; i++) {
                tx_output_ptrs[i] = &tx_outputs[i];
                found_output_ptrs[i] = &found_outputs[i];
            }
