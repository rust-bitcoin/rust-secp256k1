load("rustsecp256k1_v0_6_1_params.sage")
...
for f in sorted(results.keys()):
    ...
    first = False
    print("static const rustsecp256k1_v0_6_1_ge rustsecp256k1_v0_6_1_ge_const_g = SECP256K1_GE_CONST(")
    print("    0x%08x, 0x%08x, 0x%08x, 0x%08x," % tuple((int(G[0]) >> (32 * (7 - i))) & 0xffffffff for i in range(4)))
    print("    0x%08x, 0x%08x, 0x%08x, 0x%08x," % tuple((int(G[0]) >> (32 * (7 - i))) & 0xffffffff for i in range(4, 8)))
    print("    0x%08x, 0x%08x, 0x%08x, 0x%08x," % tuple((int(G[1]) >> (32 * (7 - i))) & 0xffffffff for i in range(4)))
    print("    0x%08x, 0x%08x, 0x%08x, 0x%08x" % tuple((int(G[1]) >> (32 * (7 - i))) & 0xffffffff for i in range(4, 8)))
    print(");")
    print("static const rustsecp256k1_v0_6_1_fe rustsecp256k1_v0_6_1_fe_const_b = SECP256K1_FE_CONST(")
