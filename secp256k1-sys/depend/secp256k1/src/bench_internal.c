...
static void bench_setup(void* arg) {
    ...
    rustsecp256k1_v0_8_1_fe_set_b32_limit(&data->fe[0], init[0]);
    rustsecp256k1_v0_8_1_fe_set_b32_limit(&data->fe[1], init[1]);
    rustsecp256k1_v0_8_1_fe_set_b32_limit(&data->fe[2], init[2]);
    rustsecp256k1_v0_8_1_fe_set_b32_limit(&data->fe[3], init[3]);
...
static void bench_scalar_split(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_8_1_scalar_split_lambda(&tmp, &data->scalar[1], &data->scalar[0]);
        j += rustsecp256k1_v0_8_1_scalar_add(&data->scalar[0], &tmp, &data->scalar[1]);
    }
    CHECK(j <= iters);
}
...
static void bench_field_is_square_var(void* arg, int iters) {
    int i, j = 0;
    bench_inv *data = (bench_inv*)arg;
    rustsecp256k1_v0_8_1_fe t = data->fe[0];

    for (i = 0; i < iters; i++) {
        j += rustsecp256k1_v0_8_1_fe_is_square_var(&t);
        rustsecp256k1_v0_8_1_fe_add(&t, &data->fe[1]);
        rustsecp256k1_v0_8_1_fe_normalize_var(&t);
    }
    CHECK(j <= iters);
}
