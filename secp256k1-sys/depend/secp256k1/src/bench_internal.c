...
typedef struct {
    rustsecp256k1_v0_1_2_scalar scalar_x, scalar_y;
    rustsecp256k1_v0_1_2_fe fe_x, fe_y;
    rustsecp256k1_v0_1_2_ge ge_x, ge_y;
    rustsecp256k1_v0_1_2_gej gej_x, gej_y;
...
void bench_setup(void* arg) {
    ...
    rustsecp256k1_v0_1_2_scalar_set_b32(&data->scalar_x, init_x, NULL);
    rustsecp256k1_v0_1_2_scalar_set_b32(&data->scalar_y, init_y, NULL);
    rustsecp256k1_v0_1_2_fe_set_b32(&data->fe_x, init_x);
    rustsecp256k1_v0_1_2_fe_set_b32(&data->fe_y, init_y);
    CHECK(rustsecp256k1_v0_1_2_ge_set_xo_var(&data->ge_x, &data->fe_x, 0));
    CHECK(rustsecp256k1_v0_1_2_ge_set_xo_var(&data->ge_y, &data->fe_y, 1));
    rustsecp256k1_v0_1_2_gej_set_ge(&data->gej_x, &data->ge_x);
    rustsecp256k1_v0_1_2_gej_set_ge(&data->gej_y, &data->ge_y);
...
void bench_scalar_add(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        j += rustsecp256k1_v0_1_2_scalar_add(&data->scalar_x, &data->scalar_x, &data->scalar_y);
    }
    CHECK(j <= iters);
}
...
void bench_scalar_negate(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_scalar_negate(&data->scalar_x, &data->scalar_x);
    }
}
...
void bench_scalar_sqr(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_scalar_sqr(&data->scalar_x, &data->scalar_x);
    }
}
...
void bench_scalar_mul(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_scalar_mul(&data->scalar_x, &data->scalar_x, &data->scalar_y);
    }
}
...
void bench_scalar_split(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_scalar_split_lambda(&data->scalar_x, &data->scalar_y, &data->scalar_x);
        j += rustsecp256k1_v0_1_2_scalar_add(&data->scalar_x, &data->scalar_x, &data->scalar_y);
    }
    CHECK(j <= iters);
}
...
void bench_scalar_inverse(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_scalar_inverse(&data->scalar_x, &data->scalar_x);
        j += rustsecp256k1_v0_1_2_scalar_add(&data->scalar_x, &data->scalar_x, &data->scalar_y);
    }
    CHECK(j <= iters);
}
...
void bench_scalar_inverse_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_scalar_inverse_var(&data->scalar_x, &data->scalar_x);
        j += rustsecp256k1_v0_1_2_scalar_add(&data->scalar_x, &data->scalar_x, &data->scalar_y);
    }
    CHECK(j <= iters);
}
...
void bench_field_normalize(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_fe_normalize(&data->fe_x);
    }
}
...
void bench_field_normalize_weak(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_fe_normalize_weak(&data->fe_x);
    }
}
...
void bench_field_mul(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_fe_mul(&data->fe_x, &data->fe_x, &data->fe_y);
    }
}
...
void bench_field_sqr(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_fe_sqr(&data->fe_x, &data->fe_x);
    }
}
...
void bench_field_inverse(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_fe_inv(&data->fe_x, &data->fe_x);
        rustsecp256k1_v0_1_2_fe_add(&data->fe_x, &data->fe_y);
    }
}
...
void bench_field_inverse_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_fe_inv_var(&data->fe_x, &data->fe_x);
        rustsecp256k1_v0_1_2_fe_add(&data->fe_x, &data->fe_y);
    }
}
...
void bench_field_sqrt(void* arg, int iters) {
    int i, j = 0;
    bench_inv *data = (bench_inv*)arg;
    rustsecp256k1_v0_1_2_fe t;

    for (i = 0; i < iters; i++) {
        t = data->fe_x;
        j += rustsecp256k1_v0_1_2_fe_sqrt(&data->fe_x, &t);
        rustsecp256k1_v0_1_2_fe_add(&data->fe_x, &data->fe_y);
    }
    CHECK(j <= iters);
}
...
void bench_group_double_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_gej_double_var(&data->gej_x, &data->gej_x, NULL);
    }
}
...
void bench_group_add_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_gej_add_var(&data->gej_x, &data->gej_x, &data->gej_y, NULL);
    }
}
...
void bench_group_add_affine(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_gej_add_ge(&data->gej_x, &data->gej_x, &data->ge_y);
    }
}
...
void bench_group_add_affine_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_gej_add_ge_var(&data->gej_x, &data->gej_x, &data->ge_y, NULL);
    }
}
...
void bench_group_jacobi_var(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        j += rustsecp256k1_v0_1_2_gej_has_quad_y_var(&data->gej_x);
    }
    CHECK(j == iters);
}
...
void bench_ecmult_wnaf(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        bits += rustsecp256k1_v0_1_2_ecmult_wnaf(data->wnaf, 256, &data->scalar_x, WINDOW_A);
        overflow += rustsecp256k1_v0_1_2_scalar_add(&data->scalar_x, &data->scalar_x, &data->scalar_y);
    }
    CHECK(overflow >= 0);
...
void bench_wnaf_const(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        bits += rustsecp256k1_v0_1_2_wnaf_const(data->wnaf, &data->scalar_x, WINDOW_A, 256);
        overflow += rustsecp256k1_v0_1_2_scalar_add(&data->scalar_x, &data->scalar_x, &data->scalar_y);
    }
    CHECK(overflow >= 0);
    CHECK(bits <= 256*iters);
...
void bench_sha256(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        rustsecp256k1_v0_1_2_sha256_write(&sha, data->data, 32);
        rustsecp256k1_v0_1_2_sha256_finalize(&sha, data->data);
    }
}
...
void bench_hmac_sha256(void* arg, int iters) {
    int i;
    bench_inv *data = (bench_inv*)arg;
    rustsecp256k1_v0_1_2_hmac_sha256 hmac;

    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_hmac_sha256_initialize(&hmac, data->data, 32);
        rustsecp256k1_v0_1_2_hmac_sha256_write(&hmac, data->data, 32);
        rustsecp256k1_v0_1_2_hmac_sha256_finalize(&hmac, data->data);
    }
}
...
void bench_rfc6979_hmac_sha256(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_rfc6979_hmac_sha256_initialize(&rng, data->data, 64);
        rustsecp256k1_v0_1_2_rfc6979_hmac_sha256_generate(&rng, data->data, 32);
    }
}
...
void bench_context_verify(void* arg, int iters) {
    ...
    (void)arg;
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_context_destroy(rustsecp256k1_v0_1_2_context_create(SECP256K1_CONTEXT_VERIFY));
    }
}
...
void bench_context_sign(void* arg, int iters) {
    ...
    (void)arg;
    for (i = 0; i < iters; i++) {
        rustsecp256k1_v0_1_2_context_destroy(rustsecp256k1_v0_1_2_context_create(SECP256K1_CONTEXT_SIGN));
    }
}
...
void bench_num_jacobi(void* arg, int iters) {
    int i, j = 0;
    bench_inv *data = (bench_inv*)arg;
    rustsecp256k1_v0_1_2_num nx, norder;

    rustsecp256k1_v0_1_2_scalar_get_num(&nx, &data->scalar_x);
    rustsecp256k1_v0_1_2_scalar_order_get_num(&norder);
    rustsecp256k1_v0_1_2_scalar_get_num(&norder, &data->scalar_y);

    for (i = 0; i < iters; i++) {
        j += rustsecp256k1_v0_1_2_num_jacobi(&nx, &norder);
    }
    CHECK(j <= iters);
}
#endif
