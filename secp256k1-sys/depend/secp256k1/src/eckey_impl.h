...
static int rustsecp256k1_v0_13_eckey_pubkey_serialize(rustsecp256k1_v0_13_ge *elem, unsigned char *pub, size_t *size, int compressed) {
    ...
    if (rustsecp256k1_v0_13_ge_is_infinity(elem)) {
        return 0;
    }
    rustsecp256k1_v0_13_fe_normalize_var(&elem->x);
    rustsecp256k1_v0_13_fe_normalize_var(&elem->y);
    rustsecp256k1_v0_13_fe_get_b32(&pub[1], &elem->x);
    if (compressed) {
        *size = 33;
        pub[0] = rustsecp256k1_v0_13_fe_is_odd(&elem->y) ? SECP256K1_TAG_PUBKEY_ODD : SECP256K1_TAG_PUBKEY_EVEN;
    } else {
        *size = 65;
        pub[0] = SECP256K1_TAG_PUBKEY_UNCOMPRESSED;
        rustsecp256k1_v0_13_fe_get_b32(&pub[33], &elem->y);
    }
    return 1;
}
