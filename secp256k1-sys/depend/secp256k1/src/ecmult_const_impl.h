 ...
 */
static void rustsecp256k1_v0_9_2_ecmult_odd_multiples_table_globalz_windowa(rustsecp256k1_v0_9_2_ge *pre, rustsecp256k1_v0_9_2_fe *globalz, const rustsecp256k1_v0_9_2_gej *a) {
    rustsecp256k1_v0_9_2_fe zr[ECMULT_TABLE_SIZE(WINDOW_A)];

    rustsecp256k1_v0_9_2_ecmult_odd_multiples_table(ECMULT_TABLE_SIZE(WINDOW_A), pre, zr, globalz, a);
    rustsecp256k1_v0_9_2_ge_table_set_globalz(ECMULT_TABLE_SIZE(WINDOW_A), pre, zr);
}
...
#define ECMULT_CONST_TABLE_GET_GE(r,pre,n,w) do { \
    ...
    int idx_n = abs_n >> 1; \
    rustsecp256k1_v0_9_2_fe neg_y; \
    VERIFY_CHECK(((n) & 1) == 1); \
    VERIFY_CHECK((n) >= -((1 << ((w)-1)) - 1)); \
    VERIFY_CHECK((n) <=  ((1 << ((w)-1)) - 1)); \
    VERIFY_SETUP(rustsecp256k1_v0_9_2_fe_clear(&(r)->x)); \
    VERIFY_SETUP(rustsecp256k1_v0_9_2_fe_clear(&(r)->y)); \
    ...
    for (m = 1; m < ECMULT_TABLE_SIZE(w); m++) { \
        ...
        rustsecp256k1_v0_9_2_fe_cmov(&(r)->x, &(pre)[m].x, m == idx_n); \
        rustsecp256k1_v0_9_2_fe_cmov(&(r)->y, &(pre)[m].y, m == idx_n); \
    } \
    (r)->infinity = 0; \
    rustsecp256k1_v0_9_2_fe_negate(&neg_y, &(r)->y, 1); \
    rustsecp256k1_v0_9_2_fe_cmov(&(r)->y, &neg_y, (n) != abs_n); \
} while(0)
 ...
 */
static int rustsecp256k1_v0_9_2_wnaf_const(int *wnaf, const rustsecp256k1_v0_9_2_scalar *scalar, int w, int size) {
    int global_sign;
    int skew;
    int word = 0;
    ...
    int flip;
    rustsecp256k1_v0_9_2_scalar s = *scalar;
    ...
    flip = rustsecp256k1_v0_9_2_scalar_is_high(&s);
    skew = flip ^ rustsecp256k1_v0_9_2_scalar_is_even(&s);
    rustsecp256k1_v0_9_2_scalar_cadd_bit(&s, 0, skew);
    global_sign = rustsecp256k1_v0_9_2_scalar_cond_negate(&s, flip);

    /* 4 */
    u_last = rustsecp256k1_v0_9_2_scalar_shr_int(&s, w);
    do {
        int even;

        /* 4.1 4.4 */
        u = rustsecp256k1_v0_9_2_scalar_shr_int(&s, w);
        /* 4.2 */
        even = ((u & 1) == 0);
    ...
    VERIFY_CHECK(rustsecp256k1_v0_9_2_scalar_is_zero(&s));
    VERIFY_CHECK(word == WNAF_SIZE_BITS(size, w));
    return skew;
}

static void rustsecp256k1_v0_9_2_ecmult_const(rustsecp256k1_v0_9_2_gej *r, const rustsecp256k1_v0_9_2_ge *a, const rustsecp256k1_v0_9_2_scalar *scalar) {
    rustsecp256k1_v0_9_2_ge pre_a[ECMULT_TABLE_SIZE(WINDOW_A)];
    rustsecp256k1_v0_9_2_ge tmpa;
    rustsecp256k1_v0_9_2_fe Z;

    int skew_1;
    rustsecp256k1_v0_9_2_ge pre_a_lam[ECMULT_TABLE_SIZE(WINDOW_A)];
    int wnaf_lam[1 + WNAF_SIZE(WINDOW_A - 1)];
    int skew_lam;
    rustsecp256k1_v0_9_2_scalar q_1, q_lam;
    int wnaf_1[1 + WNAF_SIZE(WINDOW_A - 1)];

    int i;

    if (rustsecp256k1_v0_9_2_ge_is_infinity(a)) {
    ...
    rustsecp256k1_v0_9_2_scalar_split_lambda(&q_1, &q_lam, scalar);
    skew_1   = rustsecp256k1_v0_9_2_wnaf_const(wnaf_1,   &q_1,   WINDOW_A - 1, 128);
    skew_lam = rustsecp256k1_v0_9_2_wnaf_const(wnaf_lam, &q_lam, WINDOW_A - 1, 128);
    ...
    rustsecp256k1_v0_9_2_gej_set_ge(r, a);
    rustsecp256k1_v0_9_2_ecmult_odd_multiples_table_globalz_windowa(pre_a, &Z, r);
    for (i = 0; i < ECMULT_TABLE_SIZE(WINDOW_A); i++) {
        rustsecp256k1_v0_9_2_fe_normalize_weak(&pre_a[i].y);
    }
    for (i = 0; i < ECMULT_TABLE_SIZE(WINDOW_A); i++) {
        rustsecp256k1_v0_9_2_ge_mul_lambda(&pre_a_lam[i], &pre_a[i]);
    }
    ...
    rustsecp256k1_v0_9_2_gej_set_ge(r, &tmpa);
    i = wnaf_lam[WNAF_SIZE_BITS(128, WINDOW_A - 1)];
    VERIFY_CHECK(i != 0);
    ECMULT_CONST_TABLE_GET_GE(&tmpa, pre_a_lam, i, WINDOW_A);
    rustsecp256k1_v0_9_2_gej_add_ge(r, r, &tmpa);
    /* remaining loop iterations */
    for (i = WNAF_SIZE_BITS(128, WINDOW_A - 1) - 1; i >= 0; i--) {
        int n;
        int j;
        for (j = 0; j < WINDOW_A - 1; ++j) {
            rustsecp256k1_v0_9_2_gej_double(r, r);
        }
...

    {
        ...
        rustsecp256k1_v0_9_2_ge_neg(&tmpa, &pre_a[0]);
        rustsecp256k1_v0_9_2_gej_add_ge(&tmpj, r, &tmpa);
        rustsecp256k1_v0_9_2_gej_cmov(r, &tmpj, skew_1);

        rustsecp256k1_v0_9_2_ge_neg(&tmpa, &pre_a_lam[0]);
        rustsecp256k1_v0_9_2_gej_add_ge(&tmpj, r, &tmpa);
        rustsecp256k1_v0_9_2_gej_cmov(r, &tmpj, skew_lam);
    }

    rustsecp256k1_v0_9_2_fe_mul(&r->z, &r->z, &Z);
}

static int rustsecp256k1_v0_9_2_ecmult_const_xonly(rustsecp256k1_v0_9_2_fe* r, const rustsecp256k1_v0_9_2_fe *n, const rustsecp256k1_v0_9_2_fe *d, const rustsecp256k1_v0_9_2_scalar *q, int known_on_curve) {
    ...
    rustsecp256k1_v0_9_2_fe g, i;
    rustsecp256k1_v0_9_2_ge p;
    rustsecp256k1_v0_9_2_gej rj;

    /* Compute g = (n^3 + B*d^3). */
    rustsecp256k1_v0_9_2_fe_sqr(&g, n);
    rustsecp256k1_v0_9_2_fe_mul(&g, &g, n);
    if (d) {
        rustsecp256k1_v0_9_2_fe b;
#ifdef VERIFY
        VERIFY_CHECK(!rustsecp256k1_v0_9_2_fe_normalizes_to_zero(d));
#endif
        rustsecp256k1_v0_9_2_fe_sqr(&b, d);
        VERIFY_CHECK(SECP256K1_B <= 8); /* magnitude of b will be <= 8 after the next call */
        rustsecp256k1_v0_9_2_fe_mul_int(&b, SECP256K1_B);
        rustsecp256k1_v0_9_2_fe_mul(&b, &b, d);
        rustsecp256k1_v0_9_2_fe_add(&g, &b);
        if (!known_on_curve) {
            ...
            rustsecp256k1_v0_9_2_fe_mul(&c, &g, d);
            if (!rustsecp256k1_v0_9_2_fe_is_square_var(&c)) return 0;
        }
    } else {
        rustsecp256k1_v0_9_2_fe_add_int(&g, SECP256K1_B);
        if (!known_on_curve) {
            /* g at this point equals x^3 + 7. Test if it is square. */
            if (!rustsecp256k1_v0_9_2_fe_is_square_var(&g)) return 0;
        }
    }

    /* Compute base point P = (n*g, g^2), the effective affine version of (n*g, g^2, v), which has
     * corresponding affine X coordinate n/d. */
    rustsecp256k1_v0_9_2_fe_mul(&p.x, &g, n);
    rustsecp256k1_v0_9_2_fe_sqr(&p.y, &g);
    p.infinity = 0;

    /* Perform x-only EC multiplication of P with q. */
#ifdef VERIFY
    VERIFY_CHECK(!rustsecp256k1_v0_9_2_scalar_is_zero(q));
#endif
    rustsecp256k1_v0_9_2_ecmult_const(&rj, &p, q);
#ifdef VERIFY
    VERIFY_CHECK(!rustsecp256k1_v0_9_2_gej_is_infinity(&rj));
#endif
     ...
     * (X / (Z^2*d*g)). */
    rustsecp256k1_v0_9_2_fe_sqr(&i, &rj.z);
    rustsecp256k1_v0_9_2_fe_mul(&i, &i, &g);
    if (d) rustsecp256k1_v0_9_2_fe_mul(&i, &i, d);
    rustsecp256k1_v0_9_2_fe_inv(&i, &i);
    rustsecp256k1_v0_9_2_fe_mul(r, &rj.x, &i);
