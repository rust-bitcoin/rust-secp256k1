...
static void rustsecp256k1_v0_12_ecmult_gen_compute_table(rustsecp256k1_v0_12_ge_storage* table, const rustsecp256k1_v0_12_ge* gen, int blocks, int teeth, int spacing) {
    ...
    rustsecp256k1_v0_12_ge* prec = checked_malloc(&default_error_callback, points_total * sizeof(*prec));
    rustsecp256k1_v0_12_gej* ds = checked_malloc(&default_error_callback, teeth * sizeof(*ds));
    rustsecp256k1_v0_12_gej* vs = checked_malloc(&default_error_callback, points_total * sizeof(*vs));
    rustsecp256k1_v0_12_gej u;
    size_t vs_pos = 0;
    rustsecp256k1_v0_12_scalar half;
    int block, i;

    VERIFY_CHECK(points_total > 0);

    /* u is the running power of two times gen we're working with, initially gen/2. */
    rustsecp256k1_v0_12_scalar_half(&half, &rustsecp256k1_v0_12_scalar_one);
    rustsecp256k1_v0_12_gej_set_infinity(&u);
    for (i = 255; i >= 0; --i) {
        /* Use a very simple multiplication ladder to avoid dependency on ecmult. */
        rustsecp256k1_v0_12_gej_double_var(&u, &u, NULL);
        if (rustsecp256k1_v0_12_scalar_get_bits_limb32(&half, i, 1)) {
            rustsecp256k1_v0_12_gej_add_ge_var(&u, &u, gen, NULL);
        }
    }
#ifdef VERIFY
    {
        /* Verify that u*2 = gen. */
        rustsecp256k1_v0_12_gej double_u;
        rustsecp256k1_v0_12_gej_double_var(&double_u, &u, NULL);
        VERIFY_CHECK(rustsecp256k1_v0_12_gej_eq_ge_var(&double_u, gen));
    }
#endif
    ...
    for (block = 0; block < blocks; ++block) {
        ...
        rustsecp256k1_v0_12_gej sum;
        rustsecp256k1_v0_12_gej_set_infinity(&sum);
        for (tooth = 0; tooth < teeth; ++tooth) {
            ...
            rustsecp256k1_v0_12_gej_add_var(&sum, &sum, &u, NULL);
        ...
        rustsecp256k1_v0_12_gej_neg(&vs[vs_pos++], &sum);
        ...
        for (tooth = 0; tooth < teeth - 1; ++tooth) {
            ...
            for (index = 0; index < stride; ++index, ++vs_pos) {
                rustsecp256k1_v0_12_gej_add_var(&vs[vs_pos], &vs[vs_pos - stride], &ds[tooth], NULL);
            }
        }
    }
    VERIFY_CHECK(vs_pos == points_total);

    /* Convert all points simultaneously from rustsecp256k1_v0_12_gej to rustsecp256k1_v0_12_ge. */
    rustsecp256k1_v0_12_ge_set_all_gej_var(prec, vs, points_total);
    /* Convert all points from rustsecp256k1_v0_12_ge to rustsecp256k1_v0_12_ge_storage output. */
    for (block = 0; block < blocks; ++block) {
        size_t index;
        for (index = 0; index < points; ++index) {
            VERIFY_CHECK(!rustsecp256k1_v0_12_ge_is_infinity(&prec[block * points + index]));
            rustsecp256k1_v0_12_ge_to_storage(&table[block * points + index], &prec[block * points + index]);
        }
    }
