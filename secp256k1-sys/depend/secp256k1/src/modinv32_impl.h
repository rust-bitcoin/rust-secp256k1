...
/* rustsecp256k1_v0_8_1_modinv32_inv256[i] = -(2*i+1)^-1 (mod 256) */
static const uint8_t rustsecp256k1_v0_8_1_modinv32_inv256[128] = {
...
static int32_t rustsecp256k1_v0_8_1_modinv32_divsteps_30_var(int32_t eta, uint32_t f0, uint32_t g0, rustsecp256k1_v0_8_1_modinv32_trans2x2 *t) {
    ...
    for (;;) {
        ...
        w = (g * rustsecp256k1_v0_8_1_modinv32_inv256[(f >> 1) & 127]) & m;
        /* Do so. */
        g += f * w;
        q += u * w;
        r += v * w;
 ...
 * Return: final eta
 */
static int32_t rustsecp256k1_v0_8_1_modinv32_posdivsteps_30_var(int32_t eta, uint32_t f0, uint32_t g0, rustsecp256k1_v0_8_1_modinv32_trans2x2 *t, int *jacp) {
...
static int rustsecp256k1_v0_8_1_jacobi32_maybe_var(const rustsecp256k1_v0_8_1_modinv32_signed30 *x, const rustsecp256k1_v0_8_1_modinv32_modinfo *modinfo) {
    ...
    for (count = 0; count < JACOBI32_ITERATIONS; ++count) {
        ...
        rustsecp256k1_v0_8_1_modinv32_trans2x2 t;
        eta = rustsecp256k1_v0_8_1_modinv32_posdivsteps_30_var(eta, f.v[0] | ((uint32_t)f.v[1] << 30), g.v[0] | ((uint32_t)g.v[1] << 30), &t, &jac);
        /* Update f,g using that transition matrix. */
#ifdef VERIFY
        VERIFY_CHECK(rustsecp256k1_v0_8_1_modinv32_mul_cmp_30(&f, len, &modinfo->modulus, 0) > 0); /* f > 0 */
        VERIFY_CHECK(rustsecp256k1_v0_8_1_modinv32_mul_cmp_30(&f, len, &modinfo->modulus, 1) <= 0); /* f <= modulus */
        VERIFY_CHECK(rustsecp256k1_v0_8_1_modinv32_mul_cmp_30(&g, len, &modinfo->modulus, 0) > 0); /* g > 0 */
        VERIFY_CHECK(rustsecp256k1_v0_8_1_modinv32_mul_cmp_30(&g, len, &modinfo->modulus, 1) < 0);  /* g < modulus */
#endif
        rustsecp256k1_v0_8_1_modinv32_update_fg_30_var(len, &f, &g, &t);
