...
static void bench_ecdh_setup(void* arg) {
    ...
    CHECK(rustsecp256k1_v0_6_1_ec_pubkey_parse(data->ctx, &data->point, point, sizeof(point)) == 1);
}
...
static void bench_ecdh(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        CHECK(rustsecp256k1_v0_6_1_ecdh(data->ctx, res, &data->point, data->scalar, NULL, NULL) == 1);
    }
}
...
void run_ecdh_bench(int iters, int argc, char** argv) {
    ...
    data.ctx = rustsecp256k1_v0_6_1_context_create(SECP256K1_FLAGS_TYPE_CONTEXT);
