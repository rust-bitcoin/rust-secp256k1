...
void ellswift_encoding_test_vectors_tests(void) {
    ...
    for (i = 0; (unsigned)i < sizeof(ellswift_xswiftec_inv_tests) / sizeof(ellswift_xswiftec_inv_tests[0]); ++i) {
        ...
        for (c = 0; c < 8; ++c) {
            ...
            int ret = rustsecp256k1_v0_13_ellswift_xswiftec_inv_var(&t, &testcase->x, &testcase->u, c);
            CHECK(ret == ((testcase->enc_bitmap >> c) & 1));
            if (ret) {
                rustsecp256k1_v0_13_fe x2;
                CHECK(fe_equal(&t, &testcase->encs[c]));
                rustsecp256k1_v0_13_ellswift_xswiftec_var(&x2, &testcase->u, &testcase->encs[c]);
...
void ellswift_decoding_test_vectors_tests(void) {
    ...
    for (i = 0; (unsigned)i < sizeof(ellswift_decode_tests) / sizeof(ellswift_decode_tests[0]); ++i) {
        ...
        int ret;
        ret = rustsecp256k1_v0_13_ellswift_decode(CTX, &pubkey, testcase->enc);
        CHECK(ret);
        ret = rustsecp256k1_v0_13_pubkey_load(CTX, &ge, &pubkey);
        CHECK(ret);
        CHECK(fe_equal(&testcase->x, &ge.x));
        CHECK(rustsecp256k1_v0_13_fe_is_odd(&ge.y) == testcase->odd_y);
    }
}
...
void ellswift_xdh_test_vectors_tests(void) {
    ...
    for (i = 0; (unsigned)i < sizeof(ellswift_xdh_tests_bip324) / sizeof(ellswift_xdh_tests_bip324[0]); ++i) {
        ...
        ret = rustsecp256k1_v0_13_ellswift_xdh(CTX, shared_secret,
                                     ell_a64, ell_b64,
                                     test->priv_ours,
                                     party,
                                     ...
                                     NULL);
        CHECK(ret);
        CHECK(rustsecp256k1_v0_13_memcmp_var(shared_secret, test->shared_secret, 32) == 0);
    }
}

/* Verify that rustsecp256k1_v0_13_ellswift_encode + decode roundtrips */
void ellswift_encode_decode_roundtrip_tests(void) {
    int i;
    for (i = 0; i < 1000 * COUNT; i++) {
        unsigned char rnd32[32];
        unsigned char ell64[64];
        rustsecp256k1_v0_13_ge g, g2;
        rustsecp256k1_v0_13_pubkey pubkey, pubkey2;
        /* Generate random public key and random randomizer. */
        testutil_random_ge_test(&g);
        rustsecp256k1_v0_13_pubkey_save(&pubkey, &g);
        testrand256(rnd32);
        /* Convert the public key to ElligatorSwift and back. */
        rustsecp256k1_v0_13_ellswift_encode(CTX, ell64, &pubkey, rnd32);
        rustsecp256k1_v0_13_ellswift_decode(CTX, &pubkey2, ell64);
        rustsecp256k1_v0_13_pubkey_load(CTX, &g2, &pubkey2);
        /* Compare with original. */
        CHECK(rustsecp256k1_v0_13_ge_eq_var(&g, &g2));
    }
}

/* Verify the behavior of rustsecp256k1_v0_13_ellswift_create */
void ellswift_create_tests(void) {
    ...
    for (i = 0; i < 400 * COUNT; i++) {
        ...
        rustsecp256k1_v0_13_ge dec;
        rustsecp256k1_v0_13_pubkey pub;
        unsigned char ell64[64];
        int ret;
        /* Generate random secret key and random randomizer. */
        if (i & 1) testrand256_test(auxrnd32);
        testutil_random_scalar_order_test(&sec);
        rustsecp256k1_v0_13_scalar_get_b32(sec32, &sec);
        /* Construct ElligatorSwift-encoded public keys for that key. */
        ret = rustsecp256k1_v0_13_ellswift_create(CTX, ell64, sec32, (i & 1) ? auxrnd32 : NULL);
        CHECK(ret);
        /* Decode it, and compare with traditionally-computed public key. */
        rustsecp256k1_v0_13_ellswift_decode(CTX, &pub, ell64);
        rustsecp256k1_v0_13_pubkey_load(CTX, &dec, &pub);
        rustsecp256k1_v0_13_ecmult(&res, NULL, &rustsecp256k1_v0_13_scalar_zero, &sec);
        CHECK(rustsecp256k1_v0_13_gej_eq_ge_var(&res, &dec));
    }
}

/* Verify that rustsecp256k1_v0_13_ellswift_xdh computes the right shared X coordinate */
void ellswift_compute_shared_secret_tests(void) {
    int i;
    for (i = 0; i < 800 * COUNT; i++) {
        unsigned char ell64[64], sec32[32], share32[32];
        rustsecp256k1_v0_13_scalar sec;
        rustsecp256k1_v0_13_ge dec, res;
        rustsecp256k1_v0_13_fe share_x;
        rustsecp256k1_v0_13_gej decj, resj;
        rustsecp256k1_v0_13_pubkey pub;
        int ret;
        ...
        rustsecp256k1_v0_13_ellswift_decode(CTX, &pub, ell64);
        rustsecp256k1_v0_13_pubkey_load(CTX, &dec, &pub);
        rustsecp256k1_v0_13_gej_set_ge(&decj, &dec);
        ...
        ret = rustsecp256k1_v0_13_ellswift_xdh(CTX, share32, ell64, ell64, sec32, i & 1, &ellswift_xdh_hash_x32, NULL);
        CHECK(ret);
        (void)rustsecp256k1_v0_13_fe_set_b32_limit(&share_x, share32); /* no overflow is possible */
        SECP256K1_FE_VERIFY(&share_x);
        /* Compute seckey*pubkey directly. */
        rustsecp256k1_v0_13_ecmult(&resj, &decj, &sec, NULL);
...
void ellswift_xdh_correctness_tests(void) {
    int i;
    /* Verify the joint behavior of rustsecp256k1_v0_13_ellswift_xdh */
    for (i = 0; i < 200 * COUNT; i++) {
        ...
        rustsecp256k1_v0_13_scalar seca, secb;
        ...
        rustsecp256k1_v0_13_ellswift_xdh_hash_function hash_function;
        void* data;
        int ret;
        ...
        if ((i % 3) == 0) {
            ...
            hash_function = rustsecp256k1_v0_13_ellswift_xdh_hash_function_bip324;
            data = NULL;
        } else {
            hash_function = rustsecp256k1_v0_13_ellswift_xdh_hash_function_prefix;
        ...
        rustsecp256k1_v0_13_scalar_get_b32(sec32a, &seca);
        rustsecp256k1_v0_13_scalar_get_b32(sec32b, &secb);

        /* Construct ElligatorSwift-encoded public keys for those keys. */
        /* For A: */
        ret = rustsecp256k1_v0_13_ellswift_create(CTX, ell64a, sec32a, auxrnd32a);
        CHECK(ret);
        /* For B: */
        ret = rustsecp256k1_v0_13_ellswift_create(CTX, ell64b, sec32b, auxrnd32b);
        CHECK(ret);

        /* Compute the shared secret both ways and compare with each other. */
        /* For A: */
        ret = rustsecp256k1_v0_13_ellswift_xdh(CTX, share32a, ell64a, ell64b, sec32a, 0, hash_function, data);
        CHECK(ret);
        /* For B: */
        ret = rustsecp256k1_v0_13_ellswift_xdh(CTX, share32b, ell64a, ell64b, sec32b, 1, hash_function, data);
        CHECK(ret);
        /* And compare: */
        CHECK(rustsecp256k1_v0_13_memcmp_var(share32a, share32b, 32) == 0);
        ...
        ret = rustsecp256k1_v0_13_ellswift_xdh(CTX, share32_bad, ell64a, ell64b_bad, sec32a, 0, hash_function, data);
        CHECK(ret); /* Mismatching encodings don't get detected by rustsecp256k1_v0_13_ellswift_xdh. */
        CHECK(rustsecp256k1_v0_13_memcmp_var(share32_bad, share32a, 32) != 0);
        /* For B (using a bad public key for A): */
        memcpy(ell64a_bad, ell64a, sizeof(ell64a_bad));
        testrand_flip(ell64a_bad, sizeof(ell64a_bad));
        ret = rustsecp256k1_v0_13_ellswift_xdh(CTX, share32_bad, ell64a_bad, ell64b, sec32b, 1, hash_function, data);
        CHECK(ret);
        CHECK(rustsecp256k1_v0_13_memcmp_var(share32_bad, share32b, 32) != 0);
        ...
        ret = rustsecp256k1_v0_13_ellswift_xdh(CTX, share32_bad, ell64a, ell64b, sec32a_bad, 0, hash_function, data);
        CHECK(!ret || rustsecp256k1_v0_13_memcmp_var(share32_bad, share32a, 32) != 0);
        /* For B: */
        memcpy(sec32b_bad, sec32b, sizeof(sec32b_bad));
        testrand_flip(sec32b_bad, sizeof(sec32b_bad));
        ret = rustsecp256k1_v0_13_ellswift_xdh(CTX, share32_bad, ell64a, ell64b, sec32b_bad, 1, hash_function, data);
        CHECK(!ret || rustsecp256k1_v0_13_memcmp_var(share32_bad, share32b, 32) != 0);
        ...
        if (hash_function != ellswift_xdh_hash_x32) {
            ...
            ret = rustsecp256k1_v0_13_ellswift_create(CTX, ell64b_bad, sec32b, auxrnd32b_bad);
            CHECK(ret);
            ret = rustsecp256k1_v0_13_ellswift_xdh(CTX, share32_bad, ell64a, ell64b_bad, sec32a, 0, hash_function, data);
            CHECK(ret);
            CHECK(rustsecp256k1_v0_13_memcmp_var(share32_bad, share32a, 32) != 0);
            ...
            ret = rustsecp256k1_v0_13_ellswift_create(CTX, ell64a_bad, sec32a, auxrnd32a_bad);
            CHECK(ret);
            ret = rustsecp256k1_v0_13_ellswift_xdh(CTX, share32_bad, ell64a_bad, ell64b, sec32b, 1, hash_function, data);
            CHECK(ret);
            CHECK(rustsecp256k1_v0_13_memcmp_var(share32_bad, share32b, 32) != 0);
            ...
            ret = rustsecp256k1_v0_13_ellswift_xdh(CTX, share32_bad, ell64a, ell64b, sec32a, 1, hash_function, data);
            CHECK(ret);
            CHECK(rustsecp256k1_v0_13_memcmp_var(share32_bad, share32a, 32) != 0);
            /* For B (claiming to be A): */
            ret = rustsecp256k1_v0_13_ellswift_xdh(CTX, share32_bad, ell64a, ell64b, sec32b, 0, hash_function, data);
            CHECK(ret);
            CHECK(rustsecp256k1_v0_13_memcmp_var(share32_bad, share32b, 32) != 0);
        }
    }
...
void ellswift_hash_init_tests(void) {
     ...
     * rustsecp256k1_v0_13_ellswift_sha256_init_encode has the expected
     * state. */
    rustsecp256k1_v0_13_ellswift_sha256_init_encode(&sha_optimized);
    test_sha256_tag_midstate(&sha_optimized, encode_tag, sizeof(encode_tag));

    /* Check that hash initialized by
     * rustsecp256k1_v0_13_ellswift_sha256_init_create has the expected
     * state. */
    rustsecp256k1_v0_13_ellswift_sha256_init_create(&sha_optimized);
    test_sha256_tag_midstate(&sha_optimized, create_tag, sizeof(create_tag));

    /* Check that hash initialized by
     * rustsecp256k1_v0_13_ellswift_sha256_init_bip324 has the expected
     * state. */
    rustsecp256k1_v0_13_ellswift_sha256_init_bip324(&sha_optimized);
