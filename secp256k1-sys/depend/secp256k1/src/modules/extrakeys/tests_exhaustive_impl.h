...
static void test_exhaustive_extrakeys(const rustsecp256k1_v0_11_context *ctx, const rustsecp256k1_v0_11_ge* group) {
    rustsecp256k1_v0_11_keypair keypair[EXHAUSTIVE_TEST_ORDER - 1];
    rustsecp256k1_v0_11_pubkey pubkey[EXHAUSTIVE_TEST_ORDER - 1];
    ...
    for (i = 1; i < EXHAUSTIVE_TEST_ORDER; i++) {
        ...
        CHECK(rustsecp256k1_v0_11_ec_pubkey_create(ctx, &pubkey[i - 1], buf));

        /* Construct serialized xonly_pubkey from keypair. */
        CHECK(rustsecp256k1_v0_11_keypair_xonly_pub(ctx, &xonly_pubkey[i - 1], &parities[i - 1], &keypair[i - 1]));
        ...
        CHECK(rustsecp256k1_v0_11_xonly_pubkey_parse(ctx, &xonly_pubkey[i - 1], xonly_pubkey_bytes[i - 1]));
        CHECK(rustsecp256k1_v0_11_xonly_pubkey_serialize(ctx, buf, &xonly_pubkey[i - 1]));
        CHECK(rustsecp256k1_v0_11_memcmp_var(xonly_pubkey_bytes[i - 1], buf, 32) == 0);

        /* Construct the xonly_pubkey from the pubkey, and verify it matches the same. */
        CHECK(rustsecp256k1_v0_11_xonly_pubkey_from_pubkey(ctx, &xonly_pubkey[i - 1], &parity, &pubkey[i - 1]));
        CHECK(parity == parities[i - 1]);
        CHECK(rustsecp256k1_v0_11_xonly_pubkey_serialize(ctx, buf, &xonly_pubkey[i - 1]));
        CHECK(rustsecp256k1_v0_11_memcmp_var(xonly_pubkey_bytes[i - 1], buf, 32) == 0);

        /* Compare the xonly_pubkey bytes against the precomputed group. */
        rustsecp256k1_v0_11_fe_set_b32_mod(&fe, xonly_pubkey_bytes[i - 1]);
        CHECK(rustsecp256k1_v0_11_fe_equal(&fe, &group[i].x));

        /* Check the parity against the precomputed group. */
        fe = group[i].y;
        rustsecp256k1_v0_11_fe_normalize_var(&fe);
        CHECK(rustsecp256k1_v0_11_fe_is_odd(&fe) == parities[i - 1]);

        /* Verify that the higher half is identical to the lower half mirrored. */
        if (i > EXHAUSTIVE_TEST_ORDER / 2) {
            CHECK(rustsecp256k1_v0_11_memcmp_var(xonly_pubkey_bytes[i - 1], xonly_pubkey_bytes[EXHAUSTIVE_TEST_ORDER - i - 1], 32) == 0);
