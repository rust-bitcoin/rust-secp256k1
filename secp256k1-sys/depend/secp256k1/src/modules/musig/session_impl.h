...
static int rustsecp256k1_v0_10_0_musig_ge_parse_ext(rustsecp256k1_v0_10_0_ge* ge, const unsigned char *in33) {
    ...
    return rustsecp256k1_v0_10_0_eckey_pubkey_parse(ge, in33, 33);
}
...
static int rustsecp256k1_v0_10_0_musig_secnonce_load(const rustsecp256k1_v0_10_0_context* ctx, rustsecp256k1_v0_10_0_scalar *k, rustsecp256k1_v0_10_0_ge *pk, const rustsecp256k1_v0_10_0_musig_secnonce *secnonce) {
    ...
    is_zero = rustsecp256k1_v0_10_0_scalar_is_zero(&k[0]) & rustsecp256k1_v0_10_0_scalar_is_zero(&k[1]);
...
static void rustsecp256k1_v0_10_0_musig_pubnonce_save(rustsecp256k1_v0_10_0_musig_pubnonce* nonce, const rustsecp256k1_v0_10_0_ge* ge) {
    int i;
    memcpy(&nonce->data[0], rustsecp256k1_v0_10_0_musig_pubnonce_magic, 4);
    for (i = 0; i < 2; i++) {
        rustsecp256k1_v0_10_0_ge_to_bytes(nonce->data + 4+64*i, &ge[i]);
    }
}

/* Loads two group elements from a pubnonce. Returns 1 unless the nonce wasn't
 * properly initialized */
static int rustsecp256k1_v0_10_0_musig_pubnonce_load(const rustsecp256k1_v0_10_0_context* ctx, rustsecp256k1_v0_10_0_ge* ge, const rustsecp256k1_v0_10_0_musig_pubnonce* nonce) {
    int i;

    ARG_CHECK(rustsecp256k1_v0_10_0_memcmp_var(&nonce->data[0], rustsecp256k1_v0_10_0_musig_pubnonce_magic, 4) == 0);
    for (i = 0; i < 2; i++) {
        rustsecp256k1_v0_10_0_ge_from_bytes(&ge[i], nonce->data + 4 + 64*i);
    }
    return 1;
}
...
static void rustsecp256k1_v0_10_0_musig_aggnonce_save(rustsecp256k1_v0_10_0_musig_aggnonce* nonce, const rustsecp256k1_v0_10_0_ge* ge) {
    int i;
    memcpy(&nonce->data[0], rustsecp256k1_v0_10_0_musig_aggnonce_magic, 4);
    for (i = 0; i < 2; i++) {
        rustsecp256k1_v0_10_0_ge_to_bytes_ext(&nonce->data[4 + 64*i], &ge[i]);
    }
}

static int rustsecp256k1_v0_10_0_musig_aggnonce_load(const rustsecp256k1_v0_10_0_context* ctx, rustsecp256k1_v0_10_0_ge* ge, const rustsecp256k1_v0_10_0_musig_aggnonce* nonce) {
    int i;

    ARG_CHECK(rustsecp256k1_v0_10_0_memcmp_var(&nonce->data[0], rustsecp256k1_v0_10_0_musig_aggnonce_magic, 4) == 0);
    for (i = 0; i < 2; i++) {
        rustsecp256k1_v0_10_0_ge_from_bytes_ext(&ge[i], &nonce->data[4 + 64*i]);
    }
    return 1;
}
...
int rustsecp256k1_v0_10_0_musig_pubnonce_parse(const rustsecp256k1_v0_10_0_context* ctx, rustsecp256k1_v0_10_0_musig_pubnonce* nonce, const unsigned char *in66) {
    ...
    for (i = 0; i < 2; i++) {
        if (!rustsecp256k1_v0_10_0_eckey_pubkey_parse(&ge[i], &in66[33*i], 33)) {
            return 0;
        }
        if (!rustsecp256k1_v0_10_0_ge_is_in_correct_subgroup(&ge[i])) {
            return 0;
        }
    }
    rustsecp256k1_v0_10_0_musig_pubnonce_save(nonce, ge);
    return 1;
}
...
int rustsecp256k1_v0_10_0_musig_pubnonce_serialize(const rustsecp256k1_v0_10_0_context* ctx, unsigned char *out66, const rustsecp256k1_v0_10_0_musig_pubnonce* nonce) {
    ...
    if (!rustsecp256k1_v0_10_0_musig_pubnonce_load(ctx, ge, nonce)) {
        return 0;
    }
    for (i = 0; i < 2; i++) {
        int ret;
        size_t size = 33;
        ret = rustsecp256k1_v0_10_0_eckey_pubkey_serialize(&ge[i], &out66[33*i], &size, 1);
#ifdef VERIFY
...
int rustsecp256k1_v0_10_0_musig_aggnonce_parse(const rustsecp256k1_v0_10_0_context* ctx, rustsecp256k1_v0_10_0_musig_aggnonce* nonce, const unsigned char *in66) {
    ...
    for (i = 0; i < 2; i++) {
        if (!rustsecp256k1_v0_10_0_musig_ge_parse_ext(&ge[i], &in66[33*i])) {
            return 0;
        }
    }
    rustsecp256k1_v0_10_0_musig_aggnonce_save(nonce, ge);
    return 1;
}
...
int rustsecp256k1_v0_10_0_musig_aggnonce_serialize(const rustsecp256k1_v0_10_0_context* ctx, unsigned char *out66, const rustsecp256k1_v0_10_0_musig_aggnonce* nonce) {
    ...
    if (!rustsecp256k1_v0_10_0_musig_aggnonce_load(ctx, ge, nonce)) {
        return 0;
    }
    for (i = 0; i < 2; i++) {
        rustsecp256k1_v0_10_0_musig_ge_serialize_ext(&out66[33*i], &ge[i]);
    }
    return 1;
}
...
int rustsecp256k1_v0_10_0_musig_nonce_gen_internal(const rustsecp256k1_v0_10_0_context* ctx, rustsecp256k1_v0_10_0_musig_secnonce *secnonce, rustsecp256k1_v0_10_0_musig_pubnonce *pubnonce, const unsigned char *input_nonce, const unsigned char *seckey, const rustsecp256k1_v0_10_0_pubkey *pubkey, const unsigned char *msg32, const rustsecp256k1_v0_10_0_musig_keyagg_cache *keyagg_cache, const unsigned char *extra_input32) {
    ...
    VERIFY_CHECK(!rustsecp256k1_v0_10_0_scalar_eq(&k[0], &k[1]));
    ...
    for (i = 0; i < 2; i++) {
        ...
        rustsecp256k1_v0_10_0_ge_set_gej(&nonce_pt[i], &nonce_ptj);
        rustsecp256k1_v0_10_0_declassify(ctx, &nonce_pt[i], sizeof(nonce_pt));
        rustsecp256k1_v0_10_0_scalar_clear(&k[i]);
    }
    /* nonce_pt won't be infinity because k != 0 with overwhelming probability */
    rustsecp256k1_v0_10_0_musig_pubnonce_save(pubnonce, nonce_pt);
    return ret;
}

int rustsecp256k1_v0_10_0_musig_nonce_gen(const rustsecp256k1_v0_10_0_context* ctx, rustsecp256k1_v0_10_0_musig_secnonce *secnonce, rustsecp256k1_v0_10_0_musig_pubnonce *pubnonce, const unsigned char *session_secrand32, const unsigned char *seckey, const rustsecp256k1_v0_10_0_pubkey *pubkey, const unsigned char *msg32, const rustsecp256k1_v0_10_0_musig_keyagg_cache *keyagg_cache, const unsigned char *extra_input32) {
    int ret = 1;
    unsigned char acc = 0;
    int i;
    ...
    return rustsecp256k1_v0_10_0_musig_nonce_gen_internal(ctx, secnonce, pubnonce, session_secrand32, seckey, pubkey, msg32, keyagg_cache, extra_input32);
}

int rustsecp256k1_v0_10_0_musig_nonce_gen_counter(const rustsecp256k1_v0_10_0_context* ctx, rustsecp256k1_v0_10_0_musig_secnonce *secnonce, rustsecp256k1_v0_10_0_musig_pubnonce *pubnonce, uint64_t nonrepeating_cnt, const unsigned char *seckey, const rustsecp256k1_v0_10_0_pubkey *pubkey, const unsigned char *msg32, const rustsecp256k1_v0_10_0_musig_keyagg_cache *keyagg_cache, const unsigned char *extra_input32) {
    ...
    return rustsecp256k1_v0_10_0_musig_nonce_gen_internal(ctx, secnonce, pubnonce, buf, seckey, pubkey, msg32, keyagg_cache, extra_input32);
}

static int rustsecp256k1_v0_10_0_musig_sum_nonces(const rustsecp256k1_v0_10_0_context* ctx, rustsecp256k1_v0_10_0_gej *summed_nonces, const rustsecp256k1_v0_10_0_musig_pubnonce * const* pubnonces, size_t n_pubnonces) {
    size_t i;
    int j;

    rustsecp256k1_v0_10_0_gej_set_infinity(&summed_nonces[0]);
    rustsecp256k1_v0_10_0_gej_set_infinity(&summed_nonces[1]);

    for (i = 0; i < n_pubnonces; i++) {
        rustsecp256k1_v0_10_0_ge nonce_pt[2];
        if (!rustsecp256k1_v0_10_0_musig_pubnonce_load(ctx, nonce_pt, pubnonces[i])) {
            return 0;
        }
        for (j = 0; j < 2; j++) {
            rustsecp256k1_v0_10_0_gej_add_ge_var(&summed_nonces[j], &summed_nonces[j], &nonce_pt[j], NULL);
        }
    }
    return 1;
}
...
int rustsecp256k1_v0_10_0_musig_nonce_agg(const rustsecp256k1_v0_10_0_context* ctx, rustsecp256k1_v0_10_0_musig_aggnonce  *aggnonce, const rustsecp256k1_v0_10_0_musig_pubnonce * const* pubnonces, size_t n_pubnonces) {
    rustsecp256k1_v0_10_0_gej aggnonce_ptj[2];
    ...
    if (!rustsecp256k1_v0_10_0_musig_sum_nonces(ctx, aggnonce_ptj, pubnonces, n_pubnonces)) {
        return 0;
    }
    for (i = 0; i < 2; i++) {
        rustsecp256k1_v0_10_0_ge_set_gej(&aggnonce_pt[i], &aggnonce_ptj[i]);
    }
    rustsecp256k1_v0_10_0_musig_aggnonce_save(aggnonce, aggnonce_pt);
    return 1;
}
...
static int rustsecp256k1_v0_10_0_musig_compute_noncehash(unsigned char *noncehash, rustsecp256k1_v0_10_0_ge *aggnonce, const unsigned char *agg_pk32, const unsigned char *msg) {
...
static int rustsecp256k1_v0_10_0_musig_nonce_process_internal(int *fin_nonce_parity, unsigned char *fin_nonce, rustsecp256k1_v0_10_0_scalar *b, rustsecp256k1_v0_10_0_ge *aggnonce, const unsigned char *agg_pk32, const unsigned char *msg) {
    unsigned char noncehash[32];
    rustsecp256k1_v0_10_0_ge fin_nonce_pt;
    rustsecp256k1_v0_10_0_gej fin_nonce_ptj;
    rustsecp256k1_v0_10_0_gej aggnoncej[2];

    if (!rustsecp256k1_v0_10_0_musig_compute_noncehash(noncehash, aggnonce, agg_pk32, msg)) {
        return 0;
    }

    rustsecp256k1_v0_10_0_gej_set_ge(&aggnoncej[0], &aggnonce[0]);
    rustsecp256k1_v0_10_0_gej_set_ge(&aggnoncej[1], &aggnonce[1]);
    /* fin_nonce = aggnonce[0] + b*aggnonce[1] */
    rustsecp256k1_v0_10_0_scalar_set_b32(b, noncehash, NULL);
    rustsecp256k1_v0_10_0_gej_set_infinity(&fin_nonce_ptj);
    rustsecp256k1_v0_10_0_ecmult(&fin_nonce_ptj, &aggnoncej[1], b, NULL);
    rustsecp256k1_v0_10_0_gej_add_ge_var(&fin_nonce_ptj, &fin_nonce_ptj, &aggnonce[0], NULL);
...
int rustsecp256k1_v0_10_0_musig_nonce_process(const rustsecp256k1_v0_10_0_context* ctx, rustsecp256k1_v0_10_0_musig_session *session, const rustsecp256k1_v0_10_0_musig_aggnonce  *aggnonce, const unsigned char *msg32, const rustsecp256k1_v0_10_0_musig_keyagg_cache *keyagg_cache) {
    ...
    if (!rustsecp256k1_v0_10_0_musig_aggnonce_load(ctx, aggnonce_pt, aggnonce)) {
        return 0;
    }

    if (!rustsecp256k1_v0_10_0_musig_nonce_process_internal(&session_i.fin_nonce_parity, fin_nonce, &session_i.noncecoef, aggnonce_pt, agg_pk32, msg32)) {
        return 0;
    }
...
int rustsecp256k1_v0_10_0_musig_partial_sig_verify(const rustsecp256k1_v0_10_0_context* ctx, const rustsecp256k1_v0_10_0_musig_partial_sig *partial_sig, const rustsecp256k1_v0_10_0_musig_pubnonce *pubnonce, const rustsecp256k1_v0_10_0_pubkey *pubkey, const rustsecp256k1_v0_10_0_musig_keyagg_cache *keyagg_cache, const rustsecp256k1_v0_10_0_musig_session *session) {
    ...
    if (!rustsecp256k1_v0_10_0_musig_pubnonce_load(ctx, nonce_pt, pubnonce)) {
        return 0;
    }
    rustsecp256k1_v0_10_0_gej_set_ge(&rj, &nonce_pt[1]);
    rustsecp256k1_v0_10_0_ecmult(&rj, &rj, &session_i.noncecoef, NULL);
    rustsecp256k1_v0_10_0_gej_add_ge_var(&rj, &rj, &nonce_pt[0], NULL);
