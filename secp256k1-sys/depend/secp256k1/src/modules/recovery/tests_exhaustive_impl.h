...
static void test_exhaustive_recovery_sign(const rustsecp256k1_v0_9_2_context *ctx, const rustsecp256k1_v0_9_2_ge *group) {
    int i, j, k;
    uint64_t iter = 0;
    ...
    for (i = 1; i < EXHAUSTIVE_TEST_ORDER; i++) {  /* message */
        for (j = 1; j < EXHAUSTIVE_TEST_ORDER; j++) {  /* key */
            ...
            for (k = 1; k < EXHAUSTIVE_TEST_ORDER; k++) {  /* nonce */
                ...
                rustsecp256k1_v0_9_2_fe r_dot_y_normalized;
                ...
                /* Check directly */
                rustsecp256k1_v0_9_2_ecdsa_recoverable_signature_load(ctx, &r, &s, &recid, &rsig);
                ...
                rustsecp256k1_v0_9_2_fe_normalize(&r_dot_y_normalized);
                /* Also the recovery id is flipped depending if we hit the low-s branch */
                if ((k * s) % EXHAUSTIVE_TEST_ORDER == (i + r * j) % EXHAUSTIVE_TEST_ORDER) {
                    expected_recid |= rustsecp256k1_v0_9_2_fe_is_odd(&r_dot_y_normalized);
                } else {
                    expected_recid |= !rustsecp256k1_v0_9_2_fe_is_odd(&r_dot_y_normalized);
                }
                ...
                rustsecp256k1_v0_9_2_ecdsa_recoverable_signature_convert(ctx, &sig, &rsig);
...
static void test_exhaustive_recovery_verify(const rustsecp256k1_v0_9_2_context *ctx, const rustsecp256k1_v0_9_2_ge *group) {
        ...
        for (r = 1; r < EXHAUSTIVE_TEST_ORDER; r++) {
            for (msg = 1; msg < EXHAUSTIVE_TEST_ORDER; msg++) {
                for (key = 1; key < EXHAUSTIVE_TEST_ORDER; key++) {
                    ...
                    rustsecp256k1_v0_9_2_ecdsa_recoverable_signature_save(&rsig, &r_s, &s_s, recid);
                    rustsecp256k1_v0_9_2_ecdsa_recoverable_signature_convert(ctx, &sig, &rsig);
...
static void test_exhaustive_recovery(const rustsecp256k1_v0_9_2_context *ctx, const rustsecp256k1_v0_9_2_ge *group) {
