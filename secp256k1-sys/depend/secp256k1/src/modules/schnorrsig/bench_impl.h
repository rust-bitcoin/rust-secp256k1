...
typedef struct {
    ...
    const rustsecp256k1_v0_8_1_keypair **keypairs;
...
void bench_schnorrsig_sign(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        msg[0] = i;
        msg[1] = i >> 8;
        CHECK(rustsecp256k1_v0_8_1_schnorrsig_sign_custom(data->ctx, sig, msg, MSGLEN, data->keypairs[i], NULL));
    }
}
...
void bench_schnorrsig_verify(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        CHECK(rustsecp256k1_v0_8_1_xonly_pubkey_parse(data->ctx, &pk, data->pk[i]) == 1);
        CHECK(rustsecp256k1_v0_8_1_schnorrsig_verify(data->ctx, data->sigs[i], data->msgs[i], MSGLEN, &pk));
    }
}
...
void run_schnorrsig_bench(int iters, int argc, char** argv) {
    ...
    data.keypairs = (const rustsecp256k1_v0_8_1_keypair **)malloc(iters * sizeof(rustsecp256k1_v0_8_1_keypair *));
    ...
    for (i = 0; i < iters; i++) {
        ...
        rustsecp256k1_v0_8_1_keypair *keypair = (rustsecp256k1_v0_8_1_keypair *)malloc(sizeof(*keypair));
        ...
        CHECK(rustsecp256k1_v0_8_1_keypair_create(data.ctx, keypair, sk));
        CHECK(rustsecp256k1_v0_8_1_schnorrsig_sign_custom(data.ctx, sig, msg, MSGLEN, keypair, NULL));
        CHECK(rustsecp256k1_v0_8_1_keypair_xonly_pub(data.ctx, &pk, NULL, keypair));
        CHECK(rustsecp256k1_v0_8_1_xonly_pubkey_serialize(data.ctx, pk_char, &pk) == 1);
    }
