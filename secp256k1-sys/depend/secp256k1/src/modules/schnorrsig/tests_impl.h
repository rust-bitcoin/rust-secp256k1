...
static void test_sha256_eq(const rustsecp256k1_v0_8_1_sha256 *sha1, const rustsecp256k1_v0_8_1_sha256 *sha2) {
...
static void test_schnorrsig_api(void) {
    ...
    CHECK(ecount == 4);
    CHECK(rustsecp256k1_v0_8_1_schnorrsig_sign32(STATIC_CTX, sig, msg, &keypairs[0], NULL) == 0);
    CHECK(ecount == 5);
    ...
    CHECK(ecount == 5);
    CHECK(rustsecp256k1_v0_8_1_schnorrsig_sign_custom(STATIC_CTX, sig, msg, sizeof(msg), &keypairs[0], &extraparams) == 0);
    CHECK(ecount == 6);
...
static void test_schnorrsig_bip_vectors_check_signing(const unsigned char *sk, const unsigned char *pk_serialized, const unsigned char *aux_rand, const unsigned char *msg, size_t msglen, const unsigned char *expected_sig) {
    ...
    CHECK(rustsecp256k1_v0_8_1_schnorrsig_sign_custom(CTX, sig, msg, msglen, &keypair, &extraparams));
    CHECK(rustsecp256k1_v0_8_1_memcmp_var(sig, expected_sig, 64) == 0);
    if (msglen == 32) {
        memset(sig, 0, 64);
        CHECK(rustsecp256k1_v0_8_1_schnorrsig_sign32(CTX, sig, msg, &keypair, aux_rand));
        CHECK(rustsecp256k1_v0_8_1_memcmp_var(sig, expected_sig, 64) == 0);
    }
    ...
    CHECK(rustsecp256k1_v0_8_1_schnorrsig_verify(CTX, sig, msg, msglen, &pk));
}
...
static void test_schnorrsig_bip_vectors_check_verify(const unsigned char *pk_serialized, const unsigned char *msg, size_t msglen, const unsigned char *sig, int expected) {
    ...
    CHECK(expected == rustsecp256k1_v0_8_1_schnorrsig_verify(CTX, sig, msg, msglen, &pk));
}
