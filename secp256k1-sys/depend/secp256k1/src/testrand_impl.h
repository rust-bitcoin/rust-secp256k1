...
static uint64_t rustsecp256k1_v0_13_test_state[4];
...
SECP256K1_INLINE static void testrand_seed(const unsigned char *seed16) {
    ...
    rustsecp256k1_v0_13_sha256_write(&hash, PREFIX, sizeof(PREFIX));
    rustsecp256k1_v0_13_sha256_write(&hash, seed16, 16);
    rustsecp256k1_v0_13_sha256_finalize(&hash, out32);
    for (i = 0; i < 4; ++i) {
        uint64_t s = 0;
        int j;
        for (j = 0; j < 8; ++j) s = (s << 8) | out32[8*i + j];
        rustsecp256k1_v0_13_test_state[i] = s;
    }
}
...
SECP256K1_INLINE static uint64_t testrand64(void) {
    ...
    const uint64_t result = rotl(rustsecp256k1_v0_13_test_state[0] + rustsecp256k1_v0_13_test_state[3], 23) + rustsecp256k1_v0_13_test_state[0];
    const uint64_t t = rustsecp256k1_v0_13_test_state[1] << 17;
    rustsecp256k1_v0_13_test_state[2] ^= rustsecp256k1_v0_13_test_state[0];
    rustsecp256k1_v0_13_test_state[3] ^= rustsecp256k1_v0_13_test_state[1];
    rustsecp256k1_v0_13_test_state[1] ^= rustsecp256k1_v0_13_test_state[2];
    rustsecp256k1_v0_13_test_state[0] ^= rustsecp256k1_v0_13_test_state[3];
    rustsecp256k1_v0_13_test_state[2] ^= t;
    rustsecp256k1_v0_13_test_state[3] = rotl(rustsecp256k1_v0_13_test_state[3], 45);
    return result;
}
