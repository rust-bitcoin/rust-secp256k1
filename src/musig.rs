...
use crate::{
    schnorr, Keypair, Message, PublicKey, Scalar, Secp256k1, SecretKey, Signing, Verification,
    XOnlyPublicKey,
};
...
impl MusigSecRand {
    ...
    /// Creates a new [`MusigSecRand`] with the given bytes.
    ///
    ...
    pub fn assume_unique_per_nonce_gen(inner: [u8; 32]) -> Self { MusigSecRand(inner) }

    /// Obtains the inner bytes of the [`MusigSecRand`].
    pub fn to_bytes(&self) -> [u8; 32] { self.0 }

    /// Obtains a reference to the inner bytes of the [`MusigSecRand`].
...
/// * `session_id`: [`MusigSecRand`] Uniform random identifier for this session. Each call to this
...
/// # use secp256k1_zkp::{PublicKey, Secp256k1, SecretKey, new_musig_nonce_pair, MusigSecRand};
/// # let secp = Secp256k1::new();
/// // The session id must be sampled at random. Read documentation for more details.
/// let session_id = MusigSecRand::new(&mut thread_rng());
...
impl MusigKeyAggCache {
    ...
    pub fn agg_pk(&self) -> XOnlyPublicKey { self.1 }
    ...
    /// # # [cfg(any(test, feature = "rand-std"))] {
    /// # use secp256k1_zkp::rand::{thread_rng, RngCore};
    /// # use secp256k1_zkp::{MusigKeyAggCache, Secp256k1, SecretKey, Keypair, PublicKey, MusigSecRand, Message};
    /// # let secp = Secp256k1::new();
    /// # let sk1 = SecretKey::new(&mut thread_rng());
    /// # let pub_key1 = PublicKey::from_secret_key(&secp, &sk1);
    /// # let sk2 = SecretKey::new(&mut thread_rng());
    ...
    pub fn nonce_gen<C: Signing>(
        ...
        new_musig_nonce_pair(secp, session_id, Some(self), None, pub_key, Some(msg), extra_rand)
    }

    /// Get a const pointer to the inner MusigKeyAggCache
    pub fn as_ptr(&self) -> *const ffi::MusigKeyAggCache { &self.0 }

    /// Get a mut pointer to the inner MusigKeyAggCache
    pub fn as_mut_ptr(&mut self) -> *mut ffi::MusigKeyAggCache { &mut self.0 }
}
...
impl MusigAggNonce {
    ...
    /// # # [cfg(any(test, feature = "rand-std"))] {
    /// # use secp256k1_zkp::rand::{thread_rng, RngCore};
    /// # use secp256k1_zkp::{MusigKeyAggCache, Secp256k1, SecretKey, Keypair, PublicKey, MusigSecRand, Message, MusigAggNonce};
    /// # let secp = Secp256k1::new();
    /// # let sk1 = SecretKey::new(&mut thread_rng());
    /// # let pub_key1 = PublicKey::from_secret_key(&secp, &sk1);
    /// # let sk2 = SecretKey::new(&mut thread_rng());
    ...
    /// let msg = Message::from_digest_slice(b"Public Message we want to sign!!").unwrap();
    ///
    /// let session_id1 = MusigSecRand::new(&mut thread_rng());
    ...
    /// let session_id2 = MusigSecRand::new(&mut thread_rng());
...
impl MusigSession {
    ...
    /// # # [cfg(any(test, feature = "rand-std"))] {
    /// # use secp256k1_zkp::rand::{thread_rng, RngCore};
    /// # use secp256k1_zkp::{MusigKeyAggCache, Secp256k1, SecretKey, Keypair, PublicKey, MusigSecRand, Message, MusigAggNonce, MusigSession};
    /// # let secp = Secp256k1::new();
    /// # let sk1 = SecretKey::new(&mut thread_rng());
    /// # let pub_key1 = PublicKey::from_secret_key(&secp, &sk1);
    /// # let sk2 = SecretKey::new(&mut thread_rng());
    ...
    pub fn partial_sig_agg(&self, partial_sigs: &[&MusigPartialSignature]) -> schnorr::Signature {
        let mut sig = [0u8; 64];
        unsafe {
            let partial_sigs_ref: &[*const ffi::MusigPartialSignature] = transmute::<
                &[&MusigPartialSignature],
                &[*const ffi::MusigPartialSignature],
            >(partial_sigs);
