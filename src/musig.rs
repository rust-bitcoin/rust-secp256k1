...
impl MusigKeyAggCache {
    ...
    pub fn new<C: Verification>(secp: &Secp256k1<C>, pubkeys: &[PublicKey]) -> Self {
        ...
        unsafe {
            ...
            if secp256k1_ec_pubkey_sort(cx, pubkey_ptrs.as_ptr(), pubkey_ptrs.len()) == 0 {
    ...
    /// Same as [`MusigKeyAggCache::new`], but to be used on platforms without allocator
    pub fn new_no_allocator<C: Verification>(secp: &Secp256k1<C>, pubkey_ptrs: &[*const ffi::PublicKey]) -> Self {
        let cx = secp.ctx().as_ptr();
...
impl MusigAggNonce {
    ...
    /// Same as [`MusigAggNonce::new`], but to be used on platforms without allocator
    pub fn new_no_allocator<C: Signing>(secp: &Secp256k1<C>, nonce_ptrs: &[*const ffi::MusigPubNonce]) -> Self {
        let mut aggnonce = MusigAggNonce(ffi::MusigAggNonce::new());
...
impl MusigSession {
    ...
    /// Same as [`MusigSession::new`], but to be used on platforms without allocator
    pub fn partial_sig_agg_no_allocator(&self, partial_sigs: &[*const ffi::MusigPartialSignature]) -> schnorr::Signature {
